...
<center>[d =]{style="font-family:Kalam;color:#8b1a1a;font-size:48px"}</center>
...
z <- tibble(diam = c(3.50, 4.00, 4.50, 5.0, 5.50, 6.00, 7.0, 8.0, 9.0, 10.0, 12.0) |> signif(digits = 2), 
            rad = (diam / 2)^2 * 1e-6, 
            time = rep("", 11), 
            velocity = rep("", 11)
             )
...
- You get best results when dropping the balls down the very centre of the cylinder

- When dropped in, the spheres can sometimes capture an air bubble which will slow them down. To prevent this, give each shere a light coating of liquid before dropping them in.

- Record the distance, [d]{style="font-family:Kalam;color:#8b1a1a;"}, between the rubber bands before you begin, and don't change this over the course of the experiment.

## Analysis

::::::: columns
::: {.column width="45%"}
![](images/viscosity-graph.png){height="9cm" width="8cm"}
:::
...
$V=\frac{2}{9} \frac{gr^2\left(\rho _o-\rho_L\right)\ }{\mathcal{n}}$ Where $\eta$ is the viscosity of the liquid. The units of viscosity are Pas

g is the acceleration due to gravity, $\rho_o$ and $\rho_L$ are the densities of the ball bearings and the liquid respectively. For the case of steel ball bearings and glycerol the densities are $\rho _o \; = \; 7800\; kg/m^3$ and $\rho _L \; = \; 1800\; kg/m^3$ A graph of $V$ as a function of $r^2$ is a straight line of slope

$Slope \; = \; \frac{2}{9} \frac{g\left(\rho _o-\rho_L\right)\ }{\mathcal{n}}$ $\implies \eta \; = \; \frac{2}{9} \frac{g\left(\rho _o-\rho_L\right)\ }{slope}$
