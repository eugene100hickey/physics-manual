...
\section*{RULES OF THE LABORATORY}\label{rules-of-the-laboratory}
\addcontentsline{toc}{section}{RULES OF THE LABORATORY}

\markright{RULES OF THE LABORATORY}

Laboratory sessions are 2 hours long. The student is expected to spend
all this time in the laboratory, either performing the experiment or
writing the report. Students must work in pairs when taking measurements
(the analysis should be done separately as a check). No coats or bags
are allowed in the laboratory. No eating or drinking is allowed.

\section*{LABORATORY REGISTERS}\label{laboratory-registers}
\addcontentsline{toc}{section}{LABORATORY REGISTERS}

\markright{LABORATORY REGISTERS}

The laboratory register provides a record of each student's attendance
and lists the experiments he/she has successfully completed. An 80\%
attendance is mandatory. If a student is absent due to illness a medical
certificate must be submitted to the lecturer in charge. Students will
be marked in on the attendance register when the experiment is complete
and the equipment tidied.
...
The apparatus will be found on the benches in the laboratory. Some items
must be collected (and signed for) from the lecturer supervising the
lab. These items must be returned when the experiment is complete and
signed off.

If equipment is thought to be faulty consult the lecturer in charge.
Apparatus should NEVER be moved from one bench to another. When the
experiment is complete the equipment must be tidied/cleaned and left as
it was found at the start.

\bookmarksetup{startatroot}

\chapter{The Simple Pendulum - Determination of
`g'}\label{the-simple-pendulum---determination-of-g}

The simple pendulum is used to determine the acceleration due to
gravity, g. A short pendulum swings back and forth rapidly, a long
pendulum more slowly. We will use this relationship to determine the
acceleration caused by Earth's gravity.

Take down the following in to your laboratory copy.

\subsection*{\texorpdfstring{{Title: Simple Pendulum - Determination of
`g'}}{Title: Simple Pendulum - Determination of `g'}}\label{title-simple-pendulum---determination-of-g}
\addcontentsline{toc}{subsection}{{Title: Simple Pendulum -
Determination of `g'}}

\subsection*{\texorpdfstring{{Name:}}{Name:}}\label{name}
\addcontentsline{toc}{subsection}{{Name:}}

\subsection*{\texorpdfstring{{Date:}}{Date:}}\label{date}
\addcontentsline{toc}{subsection}{{Date:}}

\subsection*{\texorpdfstring{{Partner:}}{Partner:}}\label{partner}
\addcontentsline{toc}{subsection}{{Partner:}}
...
\toprule
Guide Length (m) & Actual Length (m) & \(T_{20}(s)\) & \(T_{1}(s)\) & \(T_{1}^2(s^2)\) \\ 
...
The simple pendulum consists of a steel ball suspended by a thread
wrapped around a clamped cork.

\includegraphics[width=8cm,height=8.5cm]{images/pendulum.png}

The length of the thread can be varied. The length of the pendulum is
the distance from the corner of cork where the thread is suspended to
the centre of the ball. For each of six lengths, you will measure the
periodic time (period) of the pendulum swing.

One swing is achieved by holding the ball about 5o from centre,
releasing it, and letting it swing across and back. You will time that
swing by measuring the time for 20 swings and divide by 20.

• Fix the pendulum distance from the corner of the cork to the centre of
the ball at about 0.2m. Measure this length, \(l\), accurate to 1mm
using a metre stick. and note in the table.

• Measure the time for 20 COMPLETE swings of the pendulum. Record your
results in the table.

• Repeat this for the six lengths of pendulum from 0.2 m to 1.2m.

• When the time for 20 swings has been recorded for all lengths, divide
those values by 20 to determine the periodic time (period). Then, square
those values to determine the period squared, \(T^2\).

\section{Analysis}\label{analysis}

\includegraphics[width=8cm,height=8.5cm]{images/pendulum-graph.png}

Plot a graph of \(T^2\) against \(l\) (\(T^2\) on the y-axis) by
plotting each of the six \(T^2\) values versus their corresponding
pendulum lengths, \(l\). Draw a best fit line though the points and
determine the slope of that ``best fit line''.

Calculate the slope by picking two points on the line \((x_1,y_1)\)
close to the origin and \((x_2,y_2)\) far from the origin. Place those x
and y values into the equation to calculate the slope:
\(slope \; = \; \frac{y_2-y_1}{x_2-x_1}\)

\section{Calculation of the acceleration due to gravity,
g}\label{calculation-of-the-acceleration-due-to-gravity-g}

The equation relating the periodic time T with the length l can be shown
to be:

\(T \; = \; 2 \pi \sqrt{\frac{l}{g}}\)

where g is the acceleration due to gravity, T is measured in seconds and
l in metres.
...
This means that the slope of our \(T^2\) vs \(l\) graph must be:
...
• the main result -- repeat the value of g obtained from the end of your
calculations. Even though it is written elsewhere in your report, it's
important to repeat it here.

• the text book value -- the true value of g known from many previous
experiments

• inaccuracies -- your value for g won't be exactly the same as the text
book answer nor will your graph be a perfect straight line. We need to
try and account for these discrepancies. Pick one feature of the
experiment and investigate whether it is an issue in the accuracy of for
your results. for example, for one length of pendulum, if you repeat the
measurement of time for 20 swings, will you get the same answer as when
doing the experiment? And for that length of pendulum, will you get same
length accurate to 1mm if you measure its length a second time? You'll
need to examine the results you have already as well as gather new
...
you are thus ruling it out. Both are valid outcomes of this errors
analysis.

• improvements - based on the inaccuracy section above, can you suggest
a way in which we could make our experiment better
...
\chapter{Measurement of the Viscosity of a
Liquid}\label{measurement-of-the-viscosity-of-a-liquid}

We measure the viscosity of a liquid (glycerol) by timing how ball
bearings of different radius fall in the liquid. We then use an equation
based on Stokes law to draw a graph of velocity as a function of radius
squared.

Take down the following in to your laboratory copy.

\subsection*{\texorpdfstring{{Title: Measurement of viscosity of a
liquid by falling ball
method.}}{Title: Measurement of viscosity of a liquid by falling ball method.}}\label{title-measurement-of-viscosity-of-a-liquid-by-falling-ball-method.}
\addcontentsline{toc}{subsection}{{Title: Measurement of viscosity of a
liquid by falling ball method.}}
...
\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
Diameter, d, (mm) & \(Radius^2, r^2, (m^2)\) & \(time(s)\) & Velocity (m/s) \\ 
\midrule\addlinespace[2.5pt]
5.0 & 62.5 $\times$ 10\textsuperscript{-6} &  &  \\ 
6.0 & 90.0 $\times$ 10\textsuperscript{-6} &  &  \\ 
7.0 & 122.5 $\times$ 10\textsuperscript{-6} &  &  \\ 
9.0 & 202.5 $\times$ 10\textsuperscript{-6} &  &  \\ 
10.0 & 250.0 $\times$ 10\textsuperscript{-6} &  &  \\ 
12.0 & 360.0 $\times$ 10\textsuperscript{-6} &  &  \\ 
\bottomrule
\end{tabular*}
\end{table}

\section{Experimental Set-Up}\label{experimental-set-up-1}

\includegraphics[width=8cm,height=8.5cm]{images/viscosity.png}

The set up consists of a glass cylinder filled with glycerol. There are
two elastic bands around the cylinder that can be varied. The ball
bearings are covered in the liquid and gently let drop in the liquid. A
timer is used to measure the time for the ball bearing to fall from one
band to the other. Each ball bearing is dropped and the time taken. The
table above is used to fill in the results.\\
The velocity of the ball bearing is the distance between the bands in
meters divided by the time in seconds. That is \(v \; = \; \frac{d}{t}\)
.

\begin{itemize}
\item
  You get best results when dropping the balls down the very centre of
  the cylinder
\item
  When dropped in, the spheres can sometimes capture an air bubble which
  will slow them down. To prevent this, give each shere a light coating
  of liquid before dropping them in.
\item
  Record the distance between the rubber bands before you begin, and
  don't change this over the course of the experiment.
\end{itemize}

\section{Analysis}\label{analysis-1}

\includegraphics[width=8cm,height=9cm]{images/viscosity-graph.png}

Draw the graph as shown on the left here, with \(v(m/s)\) on the y-axis
and \(r^2(m^2)\) on the x axis. Use scientific notation for the numbers
on the axes. Don't be surprised if there is a reasonable amount of
scatter in the points. Draw a best fit line nested through the points.
Make sure the graph has a (long) descriptive title in the form
\emph{What's on y-axis vs What's on x axis and the context}.

Calculate the slope of the best fit line using the formula
\(slope \; = \; \frac{y_2-y_1}{x_2-x_1}\)

\section{Calculation of the
Viscosity}\label{calculation-of-the-viscosity}

The equation used to calculate the viscosity is given by

\(V=\frac{2}{9} \frac{gr^2\left(\rho _o-\rho_L\right)\ }{\mathcal{n}}\)
Where \(\eta\) is the viscosity of the liquid. The units of viscosity
are Pas

g is the acceleration due to gravity, \(\rho_o\) and \(\rho_L\) are the
densities of the ball bearings and the liquid respectively. For the case
of steel ball bearings and glycerol the densities are o= 7800 kg/m3 and
L= 1800 kg/m3 A graph of V as a function of r2 is a straight line of
slope
...
\begin{itemize}
\item
  \textbf{\emph{the main results}} - repeat the value of \(\eta\)
...
\item
  \textbf{\emph{the text book (manufacturers) value}} - the liquid has a
  viscosity of \(\eta \; = \; 2Pas\)
\item
  \textbf{\emph{inaccuracies}} - your value for \(\eta\) won't be
  exactly the same as the manufacturers, nor will your graph be a
  perfect straight line. We need to try and account for these
...
\section{Apparatus}\label{apparatus-2}

Graduated cylinder filled to at least 70\% with glycerol. Rubber bands
\textasciitilde{} 1cm below the top of the liquid and \textasciitilde2cm
above the base of the cylinder. Centisecond timer. Meter stick.
Magnetised steel ball bearings of sizes given in the table above.

\bookmarksetup{startatroot}

\chapter{Hooke's Law Extension of a
Spring}\label{hookes-law-extension-of-a-spring}

Every spring has a \emph{stiffness} called its \textbf{\emph{Spring
Constant}}, \(k\), measured in Newtons per metre. Stiff, rigid springs
have high values of \(k\), loose, floppy springs have low values of
\(k\). We're going to measure \(k\) by applying weights to a spring and
seeing by how much it gets longer.

Take down the following in to your laboratory copy (but not the text in
{blue}).

\subsection*{\texorpdfstring{{Title: Hooke's Law Extension of a
Spring}}{Title: Hooke's Law Extension of a Spring}}\label{title-hookes-law-extension-of-a-spring}
\addcontentsline{toc}{subsection}{{Title: Hooke's Law Extension of a
Spring}}
...
\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
Extra Mass Added (kg) & \(Position (m)\) & \(Extension (m)\) \\ 
\midrule\addlinespace[2.5pt]
0.000 & {\textcolor[HTML]{45829B}{0.641}} & {\textcolor[HTML]{8B1A1A}{0.000}} \\ 
0.100 & {\textcolor[HTML]{45829B}{0.628}} & {\textcolor[HTML]{45829B}{0.013}} \\ 
0.200 & {\textcolor[HTML]{45829B}{0.615}} & {\textcolor[HTML]{45829B}{0.026}} \\ 
0.300 &  &  \\ 
0.400 &  &  \\ 
0.500 &  &  \\ 
0.600 &  &  \\ 
...
\section{Experimental Set-Up}\label{experimental-set-up-2}

\includegraphics[width=8cm,height=10cm]{images/hookes.png}
...
Measure the position of the bottom of the weight holder on the vertical
metre stick. This is the \emph{Position} value for the table. The
\emph{Extension} value will be 0.000m

Now add an extra 100g weight and measure the new position. The
\emph{Extension} value wil now be the abolute difference between this
position and the position before we added the extra mass.

\begin{itemize}
\item
  the values in the table are for \emph{Extra Mass Added} on top of the
  original 0.300kg. So the weight holder plus \(6 \times 100g\) would be
  an extra mass added of \(0.700kg - 0.300kg = 0.400kg\).
\item
  the extension values are always positive, it's the absolute difference
  that matters.
\item
  for extension, it is always the difference between position and the
  \emph{very first} position measurement. So the extension values should
  always be getting bigger as you descend the table.
\item
  When filling out your table, pay special attention to decimal places.
  The number of decimal places for \(Position\) and \(Extension\) should
  be \(3\), we measure to the nearest mm.
\end{itemize}

\section{Analysis}\label{analysis-2}

\includegraphics[width=8cm,height=8.5cm]{images/hookes-graph.png}

Draw the graph as shown on the left here, with \(m(kg)\) on the y-axis
and \(extension(m)\) on the x axis. Draw a best fit line nested through
...
\section{Calculation of the Spring Constant,
k}\label{calculation-of-the-spring-constant-k}

Hooke's Law states that the spring extension is directly proportional to
the force making the spring stretch (in this case the weights, the
masses times gravity)
...
so the slope of our graph will be \(\frac{k}{g}\), or
\(k \; = \; g \times slope\).
...
\item
  ...
  all exactly 100g, does it matter if we start with a 0.300kg load and
  make it heavier, or start with a heavy load and decrease it\ldots..
  You'll need to examine the results you have already as well as
  gathering additional evidence by taking further measurements. Your
  idea might well be a key issue in the quality of the results we
  obtain, or it might not be and you are thus ruling it out. Both are
...
\section{Apparatus}\label{apparatus-3}

Retort stand, weight holder, 9 \(\times\) 100g weights, metre stick, 100
N/m spring. Make sure that the bottom of the weight holder is just
touching the scale on the metre stick so measuring it's position can be
done accurately.
...
\addcontentsline{toc}{subsection}{{Title: Simple Harmonic Motion of a
Spring}}
...
\section{Experimental Set-Up}\label{experimental-set-up-3}

\includegraphics[width=8cm,height=8.5cm]{images/shm-diagram3.png}
...
\includegraphics[width=8cm,height=8.5cm]{images/graph.png}
...
\section{Calculation of the Spring Constant,
k}\label{calculation-of-the-spring-constant-k-1}
...
\section{Apparatus}\label{apparatus-4}
...
\subsection*{\texorpdfstring{{Measurement of Acceleration on a Linear
Air
Track}}{Measurement of Acceleration on a Linear Air Track}}\label{measurement-of-acceleration-on-a-linear-air-track-1}
\addcontentsline{toc}{subsection}{{Measurement of Acceleration on a
Linear Air Track}}
...
\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{0.7\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 0.12\linewidth -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 0.12\linewidth -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 0.12\linewidth -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
s(m) & \(t(s)\) & \(t^2(s^2)\) \\ 
\midrule\addlinespace[2.5pt]
0.200 &  &  \\ 
...
\section{Experimental Set-Up}\label{experimental-set-up-4}

\includegraphics[width=9cm,height=6.5cm]{images/air-track.png}
...
the end of the track. The timer is started when the glider is released,
...
\includegraphics[width=7cm,height=8cm]{images/air-track-graph.png}
...
\section{Calculation of the acceleration,
...
\begin{itemize}
\item
  \textbf{\emph{the main results}} - repeat the value of \(a\) obtained
  from the end of your calculations. Even though it is written elsewhere
  in your report, it's important to repeat it here.
\item
...
\section{Apparatus}\label{apparatus-5}
...
\subsection*{\texorpdfstring{{Title: Coefficient of Restitution of a
Ping Pong
Ball}}{Title: Coefficient of Restitution of a Ping Pong Ball}}\label{title-coefficient-of-restitution-of-a-ping-pong-ball}
\addcontentsline{toc}{subsection}{{Title: Coefficient of Restitution of
a Ping Pong Ball}}

\subsection*{\texorpdfstring{{Name:}}{Name:}}\label{name-5}
\addcontentsline{toc}{subsection}{{Name:}}
...
\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{0.75\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 75.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 60.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 45.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
...
\section{Experimental Set-Up}\label{experimental-set-up-5}
...
\section{Analysis}\label{analysis-5}

\includegraphics[width=8cm,height=9cm]{images/cor-graph-3-lines.png}
...
\section{Calculation of the COR}\label{calculation-of-the-cor}
...
\section{Apparatus}\label{apparatus-6}
...
\addcontentsline{toc}{subsection}{{Measurements of Sound Frequency using
a Sonometer}}

\subsection*{\texorpdfstring{{Name:}}{Name:}}\label{name-6}
\addcontentsline{toc}{subsection}{{Name:}}

\subsection*{\texorpdfstring{{Date:}}{Date:}}\label{date-6}
\addcontentsline{toc}{subsection}{{Date:}}

\subsection*{\texorpdfstring{{Partner:}}{Partner:}}\label{partner-6}
\addcontentsline{toc}{subsection}{{Partner:}}

\subsection*{\texorpdfstring{{Data:}}{Data:}}\label{data-6}
\addcontentsline{toc}{subsection}{{Data:}}
...
\section{Experimental Set-Up}\label{experimental-set-up-6}

\includegraphics[width=7.5cm,height=7cm]{images/sonometer.png}
...
\section{Analysis}\label{analysis-6}

\includegraphics[width=7.5cm,height=7cm]{images/sonometer-graph.png}
...
\section{\texorpdfstring{Calculation of the mass per unit length,
...
\section{Apparatus}\label{apparatus-7}
...
\chapter{Focal Length of a Convex
Lens}\label{focal-length-of-a-convex-lens}
...
we will measure the focal length of a convex (converging, thicker in the
...
\addcontentsline{toc}{subsection}{{Title: Focal Length of a Convex
Lens}}

\subsection*{\texorpdfstring{{Name:}}{Name:}}\label{name-7}
\addcontentsline{toc}{subsection}{{Name:}}

\subsection*{\texorpdfstring{{Date:}}{Date:}}\label{date-7}
\addcontentsline{toc}{subsection}{{Date:}}

\subsection*{\texorpdfstring{{Partner:}}{Partner:}}\label{partner-7}
\addcontentsline{toc}{subsection}{{Partner:}}

\subsection*{\texorpdfstring{{Data:}}{Data:}}\label{data-7}
\addcontentsline{toc}{subsection}{{Data:}}

\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{0.85\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 60.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 60.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 60.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 60.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
...
\section{Experimental Set-Up}\label{experimental-set-up-7}

\pandocbounded{\includegraphics[keepaspectratio]{images/lens-setup.png}}
...
\section{Analysis}\label{analysis-7}

\includegraphics[width=8cm,height=7.5cm]{images/lens-graph.png}
...
\section{Calculation of the Focal Length,
...
\(\frac{1}{f} \; = \; \frac{1}{v}\) and the focal length will be well
...
\includegraphics[width=8cm,height=8.5cm]{images/no-parallax.png}
...
\section{Apparatus}\label{apparatus-8}
...
\chapter{Laser Diffraction}\label{laser-diffraction}

To measure the size of objects down to 1 mm we can use a meter stick.
For smaller objects, down to \(10 \mu m\), we can use a microscope. But
for smaller again we need a new method that we'll introduce in this
experiment. It relies on two optical phenomena; diffraction and
interference.

\subsection*{Diffraction}\label{diffraction}
\addcontentsline{toc}{subsection}{Diffraction}

When waves pass through a narrow aperture, they fan out on the far side,
like water waves passing through a harbour opening.

\subsection*{Interference}\label{interference}
\addcontentsline{toc}{subsection}{Interference}

Because light can act like a wave, two light beams can interfere
\emph{constructively} (when the wave crests align) or
\emph{destructively} (when a crest coincides with a trough). This will
lead to bright and dark patches in an optical pattern.

Take down the following in to your laboratory copy. Note that we're
working in millimeters today.

\subsection*{\texorpdfstring{{Title: Laser
Diffraction}}{Title: Laser Diffraction}}\label{title-laser-diffraction}
\addcontentsline{toc}{subsection}{{Title: Laser Diffraction}}

\subsection*{\texorpdfstring{{Name:}}{Name:}}\label{name-8}
\addcontentsline{toc}{subsection}{{Name:}}

\subsection*{\texorpdfstring{{Date:}}{Date:}}\label{date-8}
\addcontentsline{toc}{subsection}{{Date:}}

\subsection*{\texorpdfstring{{Partner:}}{Partner:}}\label{partner-8}
\addcontentsline{toc}{subsection}{{Partner:}}

\subsection*{\texorpdfstring{{Data:}}{Data:}}\label{data-8}
\addcontentsline{toc}{subsection}{{Data:}}

\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
Slit & D(mm) &   m   & \(x_m(mm)\) & d(mm) \\ 
\midrule\addlinespace[2.5pt]
A &  &  &  &  \\ 
B &  &  &  &  \\ 
C &  &  &  &  \\ 
...
\section{Experimental Set-Up}\label{experimental-set-up-8}

\includegraphics[width=18cm,height=10cm]{images/laser.png}

Set up the apparatus something like the sketch above. Mark out the gaps
in the diffraction pattern with the blue pen provided, as well as a mark
in the very centre of the pattern. Count the chunks between markss, this
is \(m\). Measure the distances D and \(x_m\) and note them in your
table.

Use the equation \(d \; = \; \frac{m \lambda x_m}{D}\) to calculate
d.~Repeat this for the four slits, A, B, C, and D as well as the hair
sample. For the HeNe laser, the wavelength of light is
\(\lambda \; = \; 6.33 \times 10^{-4}mm\).
...
\item
  When filling out your table, pay special attention to significant
  figures. The number of significant figures for \(d\) is dictated by
  the smallest number of significant figures in either \(D\) or \(x_m\)
\end{itemize}

\section{Microscope}\label{microscope}

\includegraphics[width=10cm,height=8cm]{images/microscope.png}

With the help of the technician, place the slide with four slits in the
microscope and use the camera software to measure each of the four
slits. Now do the same for the hair sample. Record your answers in the
table shown in the discussion section below.

\section{Discussion}\label{discussion-8}

Use the table below to capture your \textbf{\emph{main results}} and the
\textbf{\emph{textbook answers}}.

\begin{table}
\fontsize{13.0pt}{15.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\centering\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
Slit & Laser d(mm) & μscope d(mm) & Manufacturer d(mm) \\ 
\midrule\addlinespace[2.5pt]
A &  &  & 0.02 \\ 
B &  &  & 0.04 \\ 
C &  &  & 0.08 \\ 
D &  &  & 0.16 \\ 
Hair &  &  & {\cellcolor[HTML]{rgba(211,211,211,0.5)}{}} \\ 
\bottomrule
\end{tabular*}
\end{table}
...
\begin{itemize}
\item
  \textbf{\emph{inaccuracies}} - your d values won't be same for the
  laser and microscope, nor will they agree with the manufacturer's
  values for the slits. You need to explain these discrepancies. Also,
  look out for patterns in the table; e.g.~is the laser always better
  than the microscope or vice versa, or does the laser do better for
  small slits and the microscope for the larger ones\ldots..
\item
  \textbf{\emph{improvements}} - based on the inaccuracy section above,
  can you suggest a way in which we could make our experiment better?
\end{itemize}

\section{Apparatus}\label{apparatus-9}

HeNe laser. Slide with four slits of 0.02mm, 0.04mm, 0.08mm, and 0.16mm.
Slide holder. Empty slide and blutack. Blank screen. Whiteboard marker.
Microscope with USB camera and Optik measurement software installed and
interfaced.
